...
class Frame:
    ...
    """
    def __init__(self, width, height, angle,top_limit,buttom_limit):
            self.width = width
            self.height = height
            self.angle = angle
            self.tl = top_limit
            self.bl = buttom_limit
            try:
                ...
                while angle*i < 50: #maximum value of angle 
                    tempRight,tempLeft = self.get_frame_coordinate(width=self.width,h=self.height,theta=angle*i,tl=top_limit,bl=buttom_limit)
    ...
    def get_frame_coordinate(self,width,h,theta,tl,bl):
        h=bl
        height = int(h-tl) 
        print('angle = '+str(theta)+', width='+str(width)+', start h='+str(bl)+', end h='+str(height))
        ...
        for iter in range(1,int(width/2)):      #looping the horizontal axis starting from 1
            for jter in range(1,height):        #looping the vertical axis starting from 1
                ...
                if test[0]<0.01:                #if the error is small push that values of right line and left line
                    lr[iter] = [int(width/2+iter-1),int(h-jter+1-1)]
                    ll[iter] = [int(width/2-iter-1),int(h-jter+1-1)]
                    break
                elif jter==height-1:                           #else the last value will be used
                    if lr[iter-1][1] > h-tl+2:                   
    ...
    def serialize(self):
        with open(('Frame/'+str(self.width)+'x'+str(self.height)+'_'+str(self.angle)+'_'+str(self.tl)+'_'+str(self.bl)+'frame.pkl'), 'wb') as f:
            pk.dump(self, f)
            
    def deSerialize(self):
        with open(('Frame/'+str(self.width)+'x'+str(self.height)+'_'+str(self.angle)+'_'+str(self.tl)+'_'+str(self.bl)+'frame.pkl'), 'rb') as f:
