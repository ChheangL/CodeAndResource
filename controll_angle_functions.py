...
@njit
def single_side_check(temp_arrays,h):
    ...
    if a >= 4:    
        ...
        for n in right_side:
            if not n[0]==0:
                ...
                i = i + 1
        angle = calculate_side(right_side_nZero,h)
        ...
        for n in left_side:
            if not n[0]==0:
                left_side_nZero[i] = n
                i = i + 1
        angle = calculate_side(left_side_nZero,h)
...
@njit
def get_angle(boundary_points,w,h):
    ...
    for n in range(0,lenght_of_array,2):
        ...
        if a >= 6:
            ...
            temp_array[:a,1] = h - temp_array[:a,1] 
    ...
    a,b= single_side_check(cp_boundary_points,h)
    if a[0][0]:
        return 3,a[0],b
    else:
        return 4,a[0],b